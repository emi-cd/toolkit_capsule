...
# Current directory
task_name = os.path.abspath('../darknet_v4/task_capsule/')
...
file_paths = glob.glob(os.path.join('/hdd_mount/CAPSULE/TRAIN_DATA', "*.JPG"))
file_paths.sort()
...
train = random.sample(file_paths[238:], int(len(file_paths[238:])*train_rate))
test = random.sample(train, int(len(train)*(1-train_rate)))
